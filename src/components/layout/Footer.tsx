...
export function Footer() {
  const footerTargets = new Set(['hero', 'menu', 'specialites', 'about', 'gallery', 'contact']);
  ...
  return <footer className="bg-footer text-footer-text py-14 px-5"><div className="max-w-7xl mx-auto grid sm:grid-cols-2 lg:grid-cols-4 gap-10">
    ...
    <div><p className="footer-heading">Contact</p><a href={`tel:${restaurant.phone}`} className="text-footer-text hover:text-primary transition-colors">{restaurant.phoneDisplay}</a><p className="mt-3 leading-relaxed">{restaurant.openingHours[0].label}<br/>{restaurant.openingHours[0].value}</p></div>
    <div><p className="footer-heading">Navigation</p><nav className="flex flex-col items-start gap-2">{navigation.map(link => <button key={link.target} onClick={() => document.getElementById(link.target)?.scrollIntoView({behavior:'smooth'})} className="text-footer-text hover:text-primary text-sm transition-colors">{link.label}</button>)}</nav></div>
  </div>
  ...
  <div className="max-w-7xl mx-auto mt-9 pt-6 border-t border-white/15 text-xs text-footer-text/70">© {new Date().getFullYear()} {restaurant.businessName}. {restaurant.footer.rightsText}</div></footer>;
}

...
export function Navbar() {
  ...
  const Brand = () => restaurant.logo && !logoFailed ? <img src={restaurant.logo} onError={() => setLogoFailed(true)} alt={restaurant.businessName} width="1402" height="1122" className="max-h-12 w-auto max-w-[180px] object-contain"/> : <span className="font-display text-lg sm:text-xl font-black leading-none tracking-tight text-heading">{restaurant.shortName}<small className="block mt-1 font-sans text-[9px] tracking-[.3em] text-muted">{restaurant.tagline}</small></span>;
